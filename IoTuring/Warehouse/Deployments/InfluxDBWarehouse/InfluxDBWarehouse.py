...
CONFIG_KEY_ORGANIZATION = "organization"
CONFIG_KEY_TOKEN = "token"
CONFIG_KEY_BUCKET = "bucket"

class InfluxDBWarehouse(Warehouse):
    NAME = "InfluxDB"

    def Start(self) -> None:
        self.client = InfluxDBClient3(
            host=self.GetFromConfigurations(CONFIG_KEY_HOST),
            token=self.GetFromConfigurations(CONFIG_KEY_TOKEN),
            org=self.GetFromConfigurations(CONFIG_KEY_ORGANIZATION),
            )
